...
def run_job(id):
 ...
 try:
  ...
  with lock:
   ...
   data=json.loads((p/'prepared/data.json').read_text());data['route']=m['name'];data.pop('path',None);(p/'prepared/data.json').write_text(json.dumps(data,ensure_ascii=False,separators=(',',':')));m.update(status='ready',duration=data['duration'],video=bool(data['video']),warnings=data['warnings'],model_frames=len(data['frames']),error=None,decoder_version='v9-boundary-y0');save_meta(p,m)
...
for p in ROOT.iterdir():
 if p.is_dir() and ID.fullmatch(p.name) and (p/'meta.json').is_file() and (read_meta(p)['status'] in ('queued','processing') or (read_meta(p)['status']=='ready' and read_meta(p).get('decoder_version')!='v9-boundary-y0')):submit(p.name)
