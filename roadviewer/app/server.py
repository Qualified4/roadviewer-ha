  ...
  with ProgressChannel(update_progress) as channel:
   ...
   try:
    ...
    with lock:
     ...
     data=json.loads((p/'prepared/data.json').read_text());data['route']=m['name'];data.pop('path',None);(p/'prepared/data.json').write_text(json.dumps(data,ensure_ascii=False,separators=(',',':')));m.update(status='ready',manual_conversion=False,duration=data['duration'],video=(p/'qcamera.ts').is_file(),warnings=data['warnings'],model_frames=len(data['frames']),error=None,decoder_version='v21-marker-height-30cm');save_meta(p,m)
   except Exception:
...
def requeue_startup():
 ...
 for p in recording_paths():
  m=read_meta(p)
  stale=m['status']=='ready' and (m.get('decoder_version')!='v21-marker-height-30cm' or bool(m.get('video'))!=(p/'qcamera.ts').is_file() or not (p/'prepared/data.json').is_file())
