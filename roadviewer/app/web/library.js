...
변환된 분석·그래프와 재생용 임시 영상을 제거할까요? 원본 로그·TS 및 TS 없이 보관하는 MP4는 유지되며 자동으로 다시 변환되지 않습니다.`))return;
 ...
 const pin=document.createElement('button');pin.type='button';pin.textContent=m.pinned?'Unpin · 고정 해제':'Pin · 주행 고정';
 pin.onclick=async()=>{pin.disabled=true;try{await api(`api/logs/${m.id}/pin`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pinned:!m.pinned})});await refresh()}catch(e){error(e.message);pin.disabled=false}};menu.append(pin);
...
async function api(url,options={}){
 ...
 try{
  ...
  if(!response.ok){const e=Error(result.message||result.error||`요청 실패 (${response.status})`);e.retryable=response.status>=500||response.status===408||response.status===429;e.status=response.status;throw e}
  return result;
 }catch(e){
