...
function filesChanged(e){selected=[...e.target.files].filter(f=>/(^|--)(rlog\.zst|qcamera\.ts)$/.test(f.name));$('selection').textContent=selected.length?`${selected.length}개 파일 · ${(selected.reduce((a,f)=>a+f.size,0)/1048576).toFixed(1)} MB`:'지원하는 파일이 없습니다';$('upload').disabled=busy||!selected.length}
$('files').onchange=filesChanged;$('folder').onchange=filesChanged;
