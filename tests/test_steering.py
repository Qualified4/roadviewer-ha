...
class SteeringTests(unittest.TestCase):
 ...
 def test_actual_capnp_decode(self):
  ...
  messages=[]
  for name,values in [('carState',{'vEgo':10,'steeringPressed':True,'steeringAngleDeg':23}),('carControl',{'latActive':True}),('controlsState',{'activeLaneLine':True}),('carOutput',{'actuatorsOutput':{'torque':.8}}),('modelV2',{'frameId':1,'timestampEof':1_010_000_000,'laneLines':[],'laneLineProbs':[],'roadEdges':[],'roadEdgeStds':[]})]:
  ...
  with tempfile.TemporaryDirectory() as d:
   ...
   dest,data=decoder.prepare(p);s=data['frames'][0]['steering'];self.assertEqual(s['state'],'driver');self.assertEqual(s['angle'],23);self.assertEqual(s['color'],[255,255,255]);json.loads((dest/'data.json').read_text())
