...
(async()=>{
 ...
 try{
  ...
  await page.route('https://rv.test/**',async route=>{
   ...
   if(p==='/api/logs'){listReads++;return route.fulfill({json:{logs:[{id,name:'test',status:rebuilds?'queued':'ready',uploaded:1,bytes:3,prepared_bytes:7,video:false}],max_upload_mb:512,storage_used_bytes:10,concurrency:1}})}
   ...
   if(request.method()==='DELETE'){deletes++;return route.fulfill({json:{deleted:id}})}
   if(p.endsWith('/rebuild')){rebuilds++;return route.fulfill({json:{status:'queued'}})}
  ...
  assert.deepEqual(errors,[]);console.log('PASS: mobile scrolling upload, HTML proxy retry, failed-session cleanup, storage cleanup and rebuild controls');
