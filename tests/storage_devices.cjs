...
(async()=>{
 ...
 await page.getByText('장치가 연결되었습니다.',{exact:true}).waitFor();assert(await page.locator('#pairCopy').isDisabled());await page.locator('#pairClose').click();await page.getByRole('button',{name:'Revoke · 연결 해제'}).click();await page.waitForFunction(()=>document.querySelector('.device-row button').disabled);assert(revoke);
