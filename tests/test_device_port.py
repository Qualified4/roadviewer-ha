...
import start
import device_network as network

class PortTests(unittest.TestCase):
 def test_only_network_mapping_controls_activation(self):
  for value,expected in [(None,None),(18443,18443),('18099',18099)]:
   with self.subTest(value=value),patch.dict(start.os.environ,{'SUPERVISOR_TOKEN':'test-token'}),patch.object(network,'urlopen',return_value=io.BytesIO(json.dumps({'result':'ok','data':{'network':{'8443/tcp':value},'options':{'device_api_enabled':False}}}).encode())) as call:
 ...
 def test_missing_token_keeps_api_disabled(self):
  with patch.dict(start.os.environ,{},clear=True),patch.object(network,'urlopen') as call,patch.object(start.time,'sleep'):
 ...
 def test_bad_mapping_fails_closed_without_logging_credentials(self):
  for value in [True,0,65536,'bad']:
   with self.subTest(value=value),patch.dict(start.os.environ,{'SUPERVISOR_TOKEN':'private-token'}),patch.object(network,'urlopen',side_effect=lambda *a,**kw:io.BytesIO(json.dumps({'result':'ok','data':{'network':{'8443/tcp':value}}}).encode())),patch.object(start.time,'sleep'):
 ...
 def test_transient_supervisor_failure_retries(self):
  ...
  with patch.dict(start.os.environ,{'SUPERVISOR_TOKEN':'test-token'}),patch.object(network,'urlopen',side_effect=[OSError(),response]) as call,patch.object(start.time,'sleep'):
